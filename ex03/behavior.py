...
def a_star(start_node, goal_node, list_nodes):
    ...
    while open_list.size > 0:
        ...
        for i in range(len(list_nodes)):
            ...
            if checkIntersect(list_nodes[min_node.get_index()], list_nodes[i]):
                # If the neighbor is the goal_node add to the closed_list and end the search
                if list_nodes[i] == goal_node:
                    node = Node(i, 0, min_node.get_G() + euclidean_distance(list_nodes[min_node.get_index()], list_nodes[i]), min_node.get_index())
                    ...
                    for j in range(len(closed_list)):
                        ...
                        current_F = min_node.get_G() + closed_list[j].get_h()
                    ...
                    if not find:
                        ...
                        neighbor = Node(i, h, min_node.get_G() + euclidean_distance(list_nodes[min_node.get_index()], list_nodes[i]) + h, min_node.get_index())
